...
### 实际完成
- `_draw_bottom_hand()` 不再复用 `_draw_glass_card()`，手牌由专用 `_draw_hand_card_face()` 绘制。
- 卡面使用 12 段颜色插值形成顶部强调色到底部暗色的连续渐变，不绘制外边框。
- 手牌按页面内位置计算 `-10° ~ 10°` 旋转和弧形高度，形成真正扇面。
- 选中卡上浮 28px 并放大到 1.12 倍，悬停放大到 1.06 倍；命中检测同步反算旋转和缩放。
- 普通后排卡按层级叠加半透明暗色遮罩，并拥有独立下落投影；选中/悬停卡最后绘制到最上层。
- 手牌始终显示为单个连续扇面；卡牌增加时自适应缩小重叠步长，不使用翻页。
...
### 实际完成补充
- 所有手牌保持在同一个连续扇面中，数量增加时按可用宽度自动压缩重叠间距。
- 卡牌底边以窗口底部安全边距反推，修复原先 `vp.y - 105 + 145` 导致卡底天然越界的问题。
- 新获得播种卡时不在卡牌效果执行中重排数组；消费时校验拖拽索引并按本次使用的卡牌对象定位，避免数组越界或误删其他卡。
- 所有读取 `current_hand[drag_card_index]` 的交互入口均先检查索引有效性；状态失效时安全取消操作并播报，而不是使游戏崩溃。
