...
| P0 | #5 信息面板字体加大+扩散信息 | 中 | ⬜ 待实现 |
| P0 | ~~#6 结算字幕字体+颜色+时长~~ | 低 | ✅ 已完成 |
| P0 | #8 开发卡地块预览（右下角） | 中 | ⬜ 待实现 |
| P0 | #10 沙漠容量提示 | 低 | ⬜ 待实现 |
| P0 | #11 信息面板精简+行间距 | 中 | ⬜ 待实现 |
| P0 | #12 UI方角+深色边框+立体投影 | 中 | ⬜ 待实现 |
| P0 | #13 右侧UI文字溢出修复 | 低 | ⬜ 待实现 |
| P0 | ~~#14 结算字幕分散+延迟~~ | 低 | ✅ 已完成 |
| P0 | ~~#15 可放置地块白色呼吸光效+Q/E更新~~ | 中 | ✅ 已完成 |
| P0 | #16 卡牌使用条件规则文档化 | 低 | ⬜ 待实现 |
| P0 | #17 卡牌暗纹裁剪+排列修正 | 低 | ⬜ 待实现 |
| P0 | ~~#18 UI布局优化+顶部系统信息栏~~ | 中 | ✅ 已完成 |
| P1 | #4 闭合道路奖励播种卡 | 中 | ⬜ 待实现 |
| P1 | #19 道路连接两黄鹤楼变科技大厦bug | 中 | ⬜ 待修复 |
| P1 | ~~#7 彩虹位置偏移到地块缝隙~~ | 低 | ✅ 已完成 |
| P1 | #9 手牌卡牌视觉重设计 | 高 | ⬜ 待实现 |
...
### 待排查
1. `_redraw_tile` 是否重新读取 `special_buildings` 并错误匹配
2. 道路桥接 `_update_road_bridges` 是否触发了建筑类型的重新判断
3. `_logical_cell` 函数在两个相邻建筑之间是否产生冲突
