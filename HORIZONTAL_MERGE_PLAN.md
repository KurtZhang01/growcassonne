...
这里A与B、A与C之间有边框，但A与D是**对角相邻**（不同类但不共享边）。

当前方案：
- A的右边框（覆盖A-B边）+ A的下边框（覆盖A-C边）→ 在交汇点●处相交
- D没有面向A的边框（D与A不共享边）
- 交汇点●处：A的两条边框形成L形转角，**完全覆盖了A与D之间的对角缝隙**

**无需额外角部处理**。加宽后的边框（0.72）已经覆盖到交汇点附近，A的右边框和下边框在●处相交，形成完整的L形边界。

### 最终结论

加宽边框方案天然解决了L形转角问题，因为：
1. 边框足够宽，延伸到地块中心附近（±0.265）
2. 两条边框在交汇点相交，无额外缝隙
