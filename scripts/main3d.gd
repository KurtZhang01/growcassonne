...
const RULE_TICKER_TEXT := "每回合任意组合抽3张牌，随后可打出任意数量卡牌    |    道路封闭时，沿线有花朵的玩家各获得1张1级播种卡，每条闭合道路只奖励一次    |    水域使3x3范围内植物地块的正升级概率翻倍，多个水域不叠加    |    建筑使3x3范围内植物地块容积翻倍，多个建筑不叠加    |    植物地块没有基础升降级概率，只计算上下左右四邻影响，并且每回合最多变化一级    |    有道路的植物地块可以生长和接收花朵，但不会向外扩散    |    花朵扩散按相邻同色花朵占相邻总容量的比例计算    |    实际建成建筑才获得1张1级播种卡，普通地形开发不奖励    |    森林、草地、荒漠的基础容积分别为80、50、30    |    山体只能使用开发卡，缺口只能使用建筑开发卡    |    回合结算顺序为地块改变、花朵增值、花朵扩散    |    地块降级时花朵按新容量比例缩减并向下取整    |    总回合数为玩家数乘10；花朵最多者获胜，平局比较占据植物地块数    |    "
...
func _tile_hongshan_tech_surface(root: Node3D, data: Dictionary):
	var direction_index: int = int(data.get("direction", 1))
	...
	var concrete := _soft_material(Color("#aeb8b7"))

	# Both ground slabs terminate at the exact midpoint between the two tile roots.
...
func _draw_deck_probability_preview(deck_index: int, rect: Rect2, font: Font, ink: Color, muted: Color):
	...
	match clampi(deck_index, 0, 2):
		0: rows = [["1级山体", 32.8, Color("#7f8783")], ["2级山体", 24.6, Color("#8f7b70")], ["3级山体", 16.4, Color("#a8795b")], ["4级山体", 8.2, Color("#bd7446")], ["建筑开发", 18.0, Color("#c7352e")]]
		1: rows = [["1级道路", 50.0, Color("#d7b64f")], ["2级道路", 30.0, Color("#ba9343")], ["3级道路", 20.0, Color("#8c6d35")]]
		_: rows = [["台风", 20.0, Color("#5b8eb2")], ["沙尘暴", 20.0, Color("#b8894c")], ["雨季", 25.0, Color("#65a9d8")], ["旱季", 25.0, Color("#d39a48")], ["彩虹", 10.0, Color("#7faa72")]]
...
func _deck_preview_meta(deck_index: int) -> Array:
	...
	match clampi(deck_index, 0, 2):
		0: return [["本回合抽牌", draw_state], ["1级山体", "40% / 1×1"], ["2级山体", "30% / 1×2"], ["3级山体", "20% / L形"], ["4级山体", "10% / 2×2/Z/T"], ["建筑开发", "合计18% 两级均分"], ["地块生成", "森林39 草地28 荒漠19"], ["整理顺序", "开发卡按等级从左到右"]]
		1: return [["本回合抽牌", draw_state], ["1级道路", "50% / 连接2格"], ["2级道路", "30% / 连接3格"], ["3级道路", "20% / 连接4格"], ["路径规则", "连续 可转弯 可接旧路"], ["整理顺序", "道路卡按等级从左到右"]]
		_: return [["本回合抽牌", draw_state], ["天气概率", "台风20% 沙尘暴20% 雨季25% 旱季25% 彩虹10%"], ["极端天气", "持续3回合"], ["彩虹", "立即生效并清除极端天气"], ["同类天气", "不可叠加"], ["整理顺序", "台风/沙尘/雨/旱/彩虹"]]
...
func _terrain_type_meta(terrain: int) -> Array:
	match terrain:
		...
		T_FOREST: return [["外观/类型", "深绿 / 植物"], ["容积/生长", "80 / 40%"], ["四邻贡献", "升级概率+10%"], ["等级", "最高级不能再升级"], ["降级", "森林变草地"], ["花朵处理", "降级后减半取整"]]
		T_DESERT: return [["外观/类型", "黄色 / 植物"], ["容积/生长", "30 / 20%"], ["四邻贡献", "升级概率-10%"], ["等级", "最低级不能再降级"], ["升级", "荒漠变草地"], ["道路/播种", "均可"]]
